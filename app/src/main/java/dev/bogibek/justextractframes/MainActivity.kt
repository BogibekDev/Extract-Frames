package dev.bogibek.justextractframes
...
import dev.bogibek.justextractframes.adapter.Adapter
import dev.bogibek.justextractframes.adapter.ImageAdapter
import dev.bogibek.justextractframes.decoder.Frame
import dev.bogibek.justextractframes.decoder.FrameExtractor
import dev.bogibek.justextractframes.decoder.handler.IVideoFrameExtractor
import dev.bogibek.justextractframes.utils.URIPathHelper
import dev.bogibek.justextractframes.utils.Utils
import java.io.File
...
class MainActivity : AppCompatActivity(), IVideoFrameExtractor {
    ...
    private var allFrameFileFolder: File? = null
                ...
                if (dataUri != null) {
                    ...
                    executorService.execute {
                        try {
                            ...
                            this.runOnUiThread {
                                Toast.makeText(this, "Failed to extract frames", Toast.LENGTH_SHORT)
                                    .show()
                            }
                        }
                    }
...
//            secondWay()
    ...
    override fun onCurrentFrameExtracted(currentFrame: Frame) {
        ...
        allFrameFileFolder = File(this.getExternalFilesDir(null), UUID.randomUUID().toString())
    ...
    override fun onAllFrameExtracted(processedFrameCount: Int, processedTimeMs: Long) {
        ...
        this.runOnUiThread {
            updateRecycleView()
            infoTextView.text =
                "Extract $processedFrameCount frames took $processedTimeMs ms| Saving took: $totalSavingTimeMS ms"
        }
    }
    ...
    private fun setRecyclerView(list: ArrayList<Bitmap>) {
        recyclerView = findViewById(R.id.recyclerview)
        recyclerView.layoutManager = GridLayoutManager(this, 3)
        recyclerView.adapter = Adapter(list)
    }

    private fun getFramesB(): ArrayList<Bitmap> {
